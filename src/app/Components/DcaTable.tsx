...
export default function DcaTable() {
  ...
  return (
    ...
    <div className="whitespace-nowrap min-w-[600px] max-w-[800px]" >
        ...
        {portfolio.map((coin) => (
            <div key={`${coin.symbol}${coin.currency.name}`}className="portfolio-layout items-center bg-purple-100 hover:bg-purple-200 dark:bg-purple-900 dark:hover:bg-black text-sm sm:text-base p-2">
            <div className="flex items-center gap-1 sm:gap-2">
                <Image alt={`${coin.symbol}logo`} width={40} height={40} src={coin.image}/>
                <p>{coin.name} ({coin.currency.symbol})</p>
            </div>
            <p className="px-2 sm:px-5">{coin.currency.symbol}{coin.investedPrice}</p>
